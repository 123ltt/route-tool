...
import generateFile from './generate/index'
import generateRequest from './generate/request'
import { isDirectory, isFile, prefixIntersection } from './util'
...
class ToolRoute {
  ...
  async getRouteMapping () {
    ...
    const pageRoutes = new Map<string, RouteInfo>()
    const cacheAllRoutesKey: { [key: string]: boolean } = {}

    if (!await isDirectory(viewsDir)) {
      return {
  ...
  async createFile () {
    for (const item of emptyFiles) {
      ...
      if (!await isFile(filepath)) {
        await Fs.writeFile(filepath, await Fs.readFile(Path.join(__dirname, '../tpl', item.tpl), 'utf8'), 'utf8')
      }
    }
  }
}
